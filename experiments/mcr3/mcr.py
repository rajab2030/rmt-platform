"""
MCR — supervisory layer (Experiment #3)
========================================
...
evidence, audit, verification, replay protection.

Governance is EFFECT-BASED: MCR receives a preview of the would-be state
transition and records it, so decisions are tied to the actual consequential
effect rather than merely the operation name.
"""
...
class Policy:
    ...
    def __init__(self) -> None:
        ...
        self._rules: Dict[str, str] = {
            ...
            "scale": "allowed",
            "allocate_resource": "restricted",
            "set_flag": "restricted",
            "run_command": "restricted",
            "adapter_restart": "restricted",
            "compose": "restricted",
            "raw_mutate": "denied",
        }
...
class Authority:
    """Tracks authorization grants for an agent. Grants are single-use and
    time-limited to resist replay (T9)."""
    ...
    def has_valid(self, agent: str, operation: str, target: str) -> bool:
        ...
        for g in self._grants:
            if (g["agent"] == agent and g["operation"] == operation
                    and g["target"] == target and g["status"] == "approved"
                    and not g["consumed"] and (now - g["created"]) <= self.GRANT_TTL_S):
                return True
        return False

    def consume(self, agent: str, operation: str, target: str) -> bool:
        """Mark the first matching valid grant as consumed (single-use)."""
        now = time.time()
        for g in self._grants:
            if (g["agent"] == agent and g["operation"] == operation
                    and g["target"] == target and g["status"] == "approved"
...
class MCR:
    ...
    def evaluate(
        ...
        *,
        ...
        if classification == "denied":
            self._audit(operation, target, params, DENY, requester, intent, observed, effect,
        ...
        if classification == "allowed":
            self._audit(operation, target, params, ALLOW, requester, intent, observed, effect,
                        "allowed by policy")
        ...
        if classification == "restricted":
            if self.authority.has_valid(requester, operation, target):
                # Consume the grant (single-use) to resist replay.
                self.authority.consume(requester, operation, target)
                self._audit(operation, target, params, ALLOW, requester, intent, observed, effect,
                            "authorized (grant consumed)")
                return {"decision": ALLOW, "operation": operation, "target": target,
                        "classification": classification, "message": "authorized"}

            if request_auth:
                grant = self.authority.request(requester, operation, target)
                self._audit(operation, target, params, HOLD, requester, intent, observed, effect,
        ...
        self._audit(operation, target, params, STOP, requester, intent, observed, effect,
    ...
    def _audit(self, operation, target, params, decision, requester, intent, observed, effect, reason) -> None:

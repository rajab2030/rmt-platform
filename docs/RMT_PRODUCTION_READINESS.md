...
|---|---|---|---|---|---|
| **S1** | Authentication on every mutating route | **DONE + LIVE.** `app/ops/auth.py` `require_operator` on all 10 `@app.post` routes + the whole `/agent/*` router; bearer / `X-API-Key` → the operator token list; app refuses to start if enabled + unconfigured. **T0-5 (2026-09-12):** the token list moved from an `Environment=` value to a systemd `LoadCredential=` (`docs/operations/SECRETS.md`) after `systemctl show -p Environment` was found, live, exposing both production tokens to any local user (not just root) — the prior `0600` file-permission argument protected the file, not the resolved value systemd exposes via `systemctl show`. Both tokens rotated. `auth.conf` deployed against `/etc/rmt/operator_tokens.secret`; live-verified 401/200. | **READY** | — | **P0** |
...
| **V2** | CI on every change | **DONE (2026-09-08).** `backend/scripts/ci.sh` — one gate: throwaway venv built strictly from `requirements.lock.txt` (reproducible install) → `ruff check` (errors-only: `F`, `E9`; `backend/ruff.toml`; `app/core` excluded — it keeps its own 122-test gate) → the full backend suite. Exit non-zero on any step. `.github/workflows/ci.yml` calls it on push / PR to `main`/`master`. Also closes the D1 open action (clean-venv-from-lock build). **T0-4 (2026-09-09)** finished it: repo pushed to GitHub and the workflow runs green on every push to `master` (404 passed); added an explicit `import app.main` smoke step, a `concurrency` cancel group, SHA-pinned actions, a tracked `.githooks/pre-push` (runs `ci.sh --fast`, refuses a red push), and `docs/operations/CI.md`. **Branch protection (required status check) is unavailable** — a free private repo needs GitHub Pro or going public; tracked as a follow-up. | **READY** | Add a required-check rule on `master` if/when the repo goes Pro or public. | **P1** |
...
| **R3** | RMT platform recovery procedure | **DONE (2026-09-08).** The canonical base unit + the four non-secret drop-ins (`cap04-loop`, `cap05-agent`, `bind-loopback`, `hardening`) are now captured in `projects/homelab-control-center/deploy/systemd/` (+ `README.md` inventory; secret `auth.conf` stays out of git). `backend/scripts/rmt-rebuild.sh` orchestrates the cold start: prereq check (py3.12 / git / sqlite3 / rsync / docker group / systemd / caddy) → `.venv` **from `requirements.lock.txt`** → `rmt-evidence-restore.sh <E5 backup>` → `rmt_evidence_verify.py` (aborts on structural failure) → full suite → install unit + non-secret drop-ins + `daemon-reload`, then pauses for the manual secret/CA checklist before `enable --now` + a `/health` poll. `--drill DIR` runs steps 2–5 + a throwaway instance against an rsync'd copy with **zero** live-host changes. Runbook: `docs/operations/RMT_PLATFORM_RECOVERY.md`. **Scratch-dir drill PASSED (2026-09-08):** venv from lock → evidence restored from a fresh backup → `rmt_evidence_verify.py` `RESULT: OK` (only the known-benign `14be2cb0` orphan WARN) → **342 passed** in 315s → throwaway instance on `:8011` answered `/health` `{"status":"ok"}`; live service on `:8000` untouched throughout. **Genuine fresh-host rebuild PASSED (2026-09-12, T0-2 — `docs/RMT_CAPABILITIES_EVIDENCE.md` §"T0-2"):** a real, never-before-provisioned Ubuntu 24.04 LXD system container (own systemd, own filesystem) — venv from lock → evidence restored → `rmt_evidence_verify.py` `RESULT: OK` (twice) → **522 passed, 3 skipped** (the 3 are the Docker e2e tests, correctly auto-skipping — no Docker socket in the container) → the real systemd unit + all 4 non-secret drop-ins installed, `auth.conf` (throwaway token) → `enable --now` → `/health` `{"status":"ok",...}` → confirmed loopback-only bind (`bind-loopback.conf`) and `systemd-analyze security` **4.1 OK** (`hardening.conf`, matching the original D3 result) → CAP-04 loop and CAP-05 agent drop-ins both active → Docker adapter correctly fell back to `simulation` with the documented D6 warning (no Docker socket on this host). Found and fixed a real, harmless-in-practice bug in `rmt-rebuild.sh`'s step-2 summary line (`$(( ))` arithmetic context used where `$( )` command substitution was meant) — exactly the class of thing this exercise exists to catch. Caddy (S4) and cron wiring remain manual steps as documented; not exercised (this environment's network is sandboxed, worked around with an offline pip wheelhouse for the venv step — an environment limitation, not an RMT defect). | **READY** | — | **P1** |

...
|---|---|---|---|---|---|
| **S1** | Authentication on every mutating route | **DONE + LIVE.** `app/ops/auth.py` `require_operator` on all 10 `@app.post` routes + the whole `/agent/*` router; bearer / `X-API-Key` → the operator token list; app refuses to start if enabled + unconfigured. **T0-5 (2026-09-12):** the token list moved from an `Environment=` value to a systemd `LoadCredential=` (`docs/operations/SECRETS.md`) after `systemctl show -p Environment` was found, live, exposing both production tokens to any local user (not just root) — the prior `0600` file-permission argument protected the file, not the resolved value systemd exposes via `systemctl show`. Both tokens rotated. `auth.conf` deployed against `/etc/rmt-control-center/operator_tokens.secret`; live-verified 401/200. | **READY** | — | **P0** |

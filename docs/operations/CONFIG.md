...
|---|---|---|---|
| `RMT_AUTH_ENABLED` | `true` | Master switch. Accepts `1/true/yes/on` or `0/false/no/off` (case-insensitive). Blank or unknown values refuse startup; runtime changes to invalid values return 503 on authenticated routes. Explicit false is a **local-dev only** escape hatch. | `auth.conf` |
